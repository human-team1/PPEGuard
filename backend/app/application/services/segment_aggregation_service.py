    ...
    def add_frame(
        ...
        for person in people:
            aggregate = self.people.setdefault(
                person.id,
                SegmentPersonAggregate(track_id=person.id),
            )
        ...
        if self.best_frame_selector.is_better(candidate_frame, self.best_frame):
            ...
            if saved_path:
                ...
                print(
                    f"[Segment] best frame selected - session_id={session_id}, "
                    f"segment_index={segment_index}, frame_no={frame_no}, "
                    f"regex_matches={regex_match_count}, person_count={len(people)}",
                    flush=True,
                )
