...
from ...domain.ports.customer_detection_result_repository import CustomerDetectionResultRepository
...
class ProcessDetectionResultUseCase:
    ...
    def execute(self, cmd: ProcessDetectionCommand):
        ...
        # 1. 서비스 DB (기준 저장소)에 저장
        self.result_repo.save(result)

        # 2. 세션 단위 총 탐지 결과 수 증가
        ...
        # 4. 고객사 DB에 탐지 결과 릴레이 저장 (실패 시 예외를 삼키고 로깅만 하여 메인 로직에 영향이 없도록 함)
        if self.customer_result_repo:
            try:
                self.customer_result_repo.save(result)
            except Exception as e:
                logger.error(f"고객사 DB에 탐지 결과(ID: {result.id})를 저장하는 중 오류 발생: {e}", exc_info=True)
